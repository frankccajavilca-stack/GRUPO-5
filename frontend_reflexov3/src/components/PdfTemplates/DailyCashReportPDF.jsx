...
const DailyCashReportPDF = ({
  ...
  // Manejar la nueva estructura del API
  const pagosDetallados = data?.pagos_detallados || [];
  const resumenPorMetodo = data?.resumen_por_metodo || [];
  const totalGeneral = data?.total_general || 0;
  const cantidadTotalPagos = data?.cantidad_total_pagos || 0;
  
  const now = new Date();
  const fechaHora = `${date.format('DD/MM/YYYY')} - ${now.toLocaleTimeString()}`;
  const promedioPorCita = cantidadTotalPagos > 0 ? (totalGeneral / cantidadTotalPagos).toFixed(2) : 0;
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.summaryContainer}>
          <View style={styles.summaryItem}>
            ...
            <Text style={styles.summaryValue}>{cantidadTotalPagos}</Text>
          </View>
          <View style={styles.summaryItem}>
            <Text style={styles.summaryLabel}>Promedio por Pago</Text>
