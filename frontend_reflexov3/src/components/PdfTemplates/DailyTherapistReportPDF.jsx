...
const DailyTherapistReportPDF = ({ data, date, logoUrl, companyInfo }) => {
  ...
  // Función para formatear el nombre completo del terapeuta
  const formatTherapistName = (therapist) => {
    // Asegurar que therapist es un objeto válido
    if (!therapist || typeof therapist !== 'object') {
      return 'Nombre no disponible';
    }
    
    // Usar directamente el campo 'name' que viene completo del API
    if (therapist.name && typeof therapist.name === 'string') {
      return therapist.name;
    }
    
    // Fallback: construir desde las partes individuales
    const paternal = therapist.last_name_paternal || '';
    const maternal = therapist.last_name_maternal || '';
    const firstName = therapist.first_name || '';
    
    const lastNames = [paternal, maternal].filter(Boolean).join(' ');
    const fullName = [lastNames, firstName].filter(Boolean).join(', ');
